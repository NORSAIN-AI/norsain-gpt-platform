...
- Expected files presence: missing `instructions/main.md` or `actions/schema.json` produce *warnings* (not blocking),
  while missing directories or `gpt.json` produce *errors*.
...
- `scaffoldGPT()` - Creates new GPT from template (copies `agents/_template` into `agents/<name>`
...
Note about templates: the scaffolder expects templates under `agents/templates` (prefer `agents/templates/custom_gpt`).
