...
2. Updates `gpt.json` with provided metadata (name, description, author, tags, created timestamp)
3. Performs targeted placeholder replacements:
  - replaces occurrences of `[GPT Name]` in `instructions/main.md`
  - sets `info.title` and `info.description` in `actions/schema.json`
4. Sets a creation timestamp in `gpt.json`
...
- Expected files presence: missing `instructions/main.md` or `actions/schema.json` produce *warnings* (not blocking), while missing directories or `gpt.json` produce *errors*.
...
- `scaffoldGPT()` - Creates new GPT from template (copies `agents/_template` into `agents/<name>` and applies metadata and replacements)
